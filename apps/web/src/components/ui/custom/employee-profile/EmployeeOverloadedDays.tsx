...
const EmployeeOverloadedDays = ({ days }: IEmployeeOverloadedDaysProps) => {
  return (
    <div className="flex min-h-0 flex-col border bg-sidebar p-3 xl:col-span-6 xl:row-span-1">
      <h2 className="font-bold">Перегруженные дни</h2>
      <div className="mt-3 grid min-h-0 flex-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
        {days.map((day) => (
          <div key={day.id} className="flex min-w-0 flex-col border p-3">
